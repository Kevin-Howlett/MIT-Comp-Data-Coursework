...
#Script creates classes of viruses and patients and simulates (through plots) the 
#population dynamics of viruses with and without antibiotics.
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for i in range(300):
            ...
            population.append(float(Patient.getTotalPop(patient)))
    for item in population:
        item /= float(numTrials)
    pylab.figure()
    pylab.plot(population, list(range(1,301)))
...
class TreatedPatient(Patient):
    ...
    def __init__(self, viruses, maxPop):
        ...
        self.maxPop = maxPop
        self.prescriptions = []
        Patient.TreatedPatient(self, self.viruses, self.maxPop)
    ...
    def update(self):
        ...
        if popDensity < 1:
            ...
            for virus in virusesCopy:
                try:
                    self.viruses.append(virus.reproduce(popDensity, self.getPresecriptions))
                except NoChildException:
                    pass
        return len(self.getViruses())
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(numTrials):
        ...
        for timestep in range(numTimesteps//2):
            ...
            resistantVirusPopulationPerTrial[timestep] += (patient.getResistPop(['guttagonol']))
    ...
    for timestep in virusPopulationPerTrial:
        ...
        avgResistantVirusPopulation[timestep] = (sum(resistantVirusPopulationPerTrial[timestep]) / len(resistantVirusPopulationPerTrial[timestep]))
    
    pylab.plot(list(avgVirusPopulation.values()), label = "Average total virus population")
    pylab.plot(list(avgResistantVirusPopulation.values()), label = "Average population of guttagonol-resistant virus")
